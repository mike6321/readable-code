...
public class MinesweeperGame {
    ...
    private static boolean isAllCellOpened() {
        return Arrays.stream(BOARD2)
                ...
                .noneMatch(cell -> cell.equalsSign(OPENDED_CELL_SIGN));
    }
    ...
    private static void initializeGame() {
        for (int row = 0; row < BOARD_ROW_SIZE; row++) {
            for (int col = 0; col < BOARD_COLUMN_SIZE; col++) {
                BOARD2[row][col] = Cell.of(CLOSED_CELL_SIGN);
            }
        }
    ...
    private static int countNearbyLandMins(int row, int col) {
        int count = 0;
    ...
    private static void open(int row, int col) {
        ...
        if (!BOARD2[row][col].doesNotEqualsSign(CLOSED_CELL_SIGN)) {
            return;
        }
        if (isLandMineCell(row, col)) {
            return;
        ...
        if (NEARBY_LAND_MINE_COUNTS[row][col] != 0) {
            BOARD2[row][col] = Cell.of(String.valueOf(NEARBY_LAND_MINE_COUNTS[row][col]));
            return;
        } else {
            BOARD2[row][col] = Cell.of(OPENDED_CELL_SIGN);
        }
        open(row - 1, col - 1);
        open(row - 1, col);
        open(row - 1, col + 1);
